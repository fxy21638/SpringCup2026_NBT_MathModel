...
采用**连续百分位法**构建综合 AQI（0~100 量纲）：

- 对 5 种参考污染物（CO, NMHC, C6H6, NOx, NO2），各自计算经验 CDF 分位数（0~1 连续值）
- 子指数 = 分位数 × 100
- **AQI = max(各子指数)**，体现"最差污染物决定整体"的环境科学惯例
- 等级划分：优(0-20)、良(20-40)、轻度(40-60)、中度(60-80)、重度(80-100)

优势：AQI 连续分布，无天花板效应；纯数据驱动，可复现。
...
- **每折独立 StandardScaler**，避免数据泄露
- **按时间顺序划分**训练/测试集，真实反映泛化性能
- 基线对比：线性回归（同特征、同验证方法）

关键设计原则：
- 仅使用传感器读数 + 气象 + 时间特征，不使用真实污染物浓度
- 不使用 AQI 滞后特征（避免标签泄露）
- 时间序列安全的交叉验证
...
| 模型 | R² | RMSE | MAE |
|------|-----|------|-----|
| 线性回归 | -0.13 | 15.27 | 12.07 |
| XGBoost | **0.54 +/- 0.35** | **10.68 +/- 4.69** | **7.80 +/- 3.95** |

XGBoost 5 折交叉验证详细结果：

| Fold | R² | RMSE | 训练/测试样本 |
|------|-----|------|-------------|
| 1 | 0.753 | 7.49 | 1562/1559 |
| 2 | 0.765 | 7.00 | 3121/1559 |
| 3 | 0.462 | 12.69 | 4680/1559 |
| 4 | 0.778 | 8.19 | 6239/1559 |
| 5 | -0.045 | 18.03 | 7798/1559 |
