...
- NMHC 原始数据缺失率 90.2%（8126 小时连续缺口），采用间隙感知填充：
  - ≤6 小时缺口 → 线性插值（传感器短暂掉线）
  - >6 小时缺口 → 前向填充（保守假设浓度不变，填充后子指数保持稳定）
- 子指数 = 分位数 × 100
...
| 模型 | R² | RMSE | MAE |
|------|-----|------|-----|
| 线性回归 | 0.51 | 17.71 | 13.83 |
| XGBoost | **0.56 +/- 0.29** | **16.49 +/- 5.66** | **11.54 +/- 3.02** |

XGBoost 5 折交叉验证详细结果：

| Fold | R² | RMSE | 训练/测试样本 |
|------|-----|------|-------------|
| 1 | 0.802 | 13.26 | 1562/1559 |
| 2 | 0.257 | 24.29 | 3121/1559 |
| 3 | 0.240 | 20.67 | 4680/1559 |
| 4 | 0.762 | 11.34 | 6239/1559 |
| 5 | 0.751 | 12.87 | 7798/1559 |
